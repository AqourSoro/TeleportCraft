...
public class TileEntityStamper extends TileEntity implements ITickable, IElectricConsumerBlock {
	...
	private int totalTime = 50, stampingTime = 0, energy = storage.getEnergyStored();
	...
	public void update() 
	{
		...
		if(!this.world.isRemote)
		{
			...
			{
				ItemStack result = StamperRecipes.instance().getProducingResult(inStack1, inStack2, inStack3); //may still have problem
				...
				if(result != ItemStack.EMPTY && handler.insertItem(3, result, true) == ItemStack.EMPTY)
				{
					...
					if(energy >= OUTPUT_RATE && storage.canExtract())
					{
						...
						if(++ stampingTime >= totalTime)
						{
							stampingTime = 0;
							inStack1 = handler.extractItem(0, 1, false);
							inStack2 = handler.extractItem(1, 1, false);
							result = StamperRecipes.instance().getProducingResult(inStack1, inStack2, inStack3).copy();
		...
		if(stampingTime > 0 && isWorking == false)
		{
			BlockStamper.setState(true, world, pos);
			isWorking = true;
		}
		if(stampingTime == 0 && isWorking == true) 
		{
			BlockStamper.setState(false, world, pos);
			isWorking = false;
		}
	}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound)
	{
		...
		compound.setInteger("stampingTime", stampingTime);
	...
	public void readFromNBT(NBTTagCompound compound) 
	{
		...
		stampingTime = compound.getInteger("stampingTime");
	...
	{
		return new TextComponentTranslation("container.stamper");
	}
