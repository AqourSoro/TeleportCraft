...
public class ContainerMythiniumGenerator extends Container
{
	private final TileEntityMythiniumGenerator tileentity;
	private int energy, cookTime;
	
	public ContainerMythiniumGenerator(InventoryPlayer player, TileEntityMythiniumGenerator tileentity) 
	{
		this.tileentity = tileentity;
		IItemHandler handler = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
		
		this.addSlotToContainer(new SlotItemHandler(handler, 0, 80, 33));
		//use slotitemhandler to interact with itemstackhandler
