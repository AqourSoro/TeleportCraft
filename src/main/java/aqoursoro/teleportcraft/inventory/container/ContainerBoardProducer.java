...
public class ContainerBoardProducer extends Container {

private TileEntityBoardProducer tileentity;
	
	
	private int producingTime, energy;
	
	//protected SlotItemHandler input, output, battery;
	
	public ContainerBoardProducer(@Nonnull InventoryPlayer player, @Nonnull TileEntityBoardProducer tileentity) 
	{
		this.tileentity = tileentity;
		
		IItemHandler handler = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
		
		this.addSlotToContainer(new SlotItemHandler(handler, 0, 43, 36));//stack1
		this.addSlotToContainer(new SlotItemHandler(handler, 1, 72, 12));//stack2
		
		//output slot
		this.addSlotToContainer(new SlotItemHandler(handler, 2, 104, 34) 
		{
			@Override
	...
	public void detectAndSendChanges() 
	{
		...
		for(int i = 0; i < this.listeners.size(); i++) 
		{
			...
			if(producingTime != tileentity.getField(0))
			{
		...
		producingTime = tileentity.getField(0);
	...
	public ItemStack transferStackInSlot(@Nonnull EntityPlayer playerIn, @Nonnull final int index) 
	{
		...
		if (slot != null && slot.getHasStack())
        {
	        ...
	        {
	        	if(BoardProducerRecipes.instance().haveProducingResult(newStack))
	        	{
